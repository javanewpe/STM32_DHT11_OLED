...
/*
 在Cortex-M里面有一个外设叫DWT(Data Watchpoint and Trace)，该外设有一个32位的寄存器叫CYCCNT，
 它是一个向上的计数器，记录的是内核时钟运行的个数，假设内核频率为72M，
 内核跳一次的时间大概为1/72M=14ns（最长能记录的时间为：60s=2 的32 次方/72000000）。
 ...
 4、CYCCNT和us互相转换：
 1/SystemCoreClock*CYCCNT（S）  = 1000/SystemCoreClock*CYCCNT（MS）= 1000000/SystemCoreClock*CYCCNT（US） = (X)US
